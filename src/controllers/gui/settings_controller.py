        ...
        if key == "AUDIO_BOT":
            if isinstance(value, str) and value.startswith("@CrazyMitaAIbot"):
                self.event_bus.emit(Events.GUI.SHOW_INFO_MESSAGE, {
                    "title": t("controllers.settings.audio_bot_info_title"),
                    "message": t("controllers.settings.audio_bot_info_message")
                })
            ...
            try:
                ...
                logger.info(t("controllers.settings.font_size_changed", font_size=font_size))
            except ValueError:
                logger.warning(t("controllers.settings.font_size_invalid", value=value))
            except Exception as e:
                logger.error(t("controllers.settings.font_size_error", e=e))

        elif key in ["SHOW_CHAT_TIMESTAMPS", "MAX_CHAT_HISTORY_DISPLAY", "HIDE_CHAT_TAGS"]:
            self.event_bus.emit(Events.GUI.RELOAD_CHAT_HISTORY)
            logger.info(t("controllers.settings.settings_changed", key=key, value=value))
