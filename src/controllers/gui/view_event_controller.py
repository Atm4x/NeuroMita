...
class ViewEventController(BaseController):
    ...
    def _on_history_loaded_event(self, event: Event):
        logger.debug(_("ViewEventController: получено ghost событие history_loaded, транслируем в view", "ViewEventController: received ghost event history_loaded, translating to view"))
        if self.view:
            self.view.history_loaded_signal.emit(event.data)
    
    def _on_more_history_loaded_event(self, event: Event):
        logger.debug(_("ViewEventController: получено ghost событие more_history_loaded, транслируем в view", "ViewEventController: received ghost event more_history_loaded, translating to view"))
        if self.view:
            self.view.more_history_loaded_signal.emit(event.data)
    
    def _on_model_initialized_event(self, event: Event):
        logger.debug(_("ViewEventController: получено событие model_initialized, транслируем в view", "ViewEventController: received event model_initialized, translating to view"))
        if self.view:
            self.view.model_initialized_signal.emit(event.data)
    
    def _on_model_init_cancelled_event(self, event: Event):
        logger.debug(_("ViewEventController: получено событие model_init_cancelled, транслируем в view", "ViewEventController: received event model_init_cancelled, translating to view"))
        if self.view:
            self.view.model_init_cancelled_signal.emit(event.data)
    
    def _on_model_init_failed_event(self, event: Event):
        logger.debug(_("ViewEventController: получено событие model_init_failed, транслируем в view", "ViewEventController: received event model_init_failed, translating to view"))
        if self.view:
            self.view.model_init_failed_signal.emit(event.data)
    
    def _on_reload_prompts_success_event(self, event: Event):
        logger.debug(_("ViewEventController: получено событие reload_prompts_success, транслируем в view", "ViewEventController: received event reload_prompts_success, translating to view"))
        if self.view:
            self.view.reload_prompts_success_signal.emit()
    
    def _on_reload_prompts_failed_event(self, event: Event):
        logger.debug(_("ViewEventController: получено событие reload_prompts_failed, транслируем в view", "ViewEventController: received event reload_prompts_failed, translating to view"))
        if self.view:
            self.view.reload_prompts_failed_signal.emit(event.data)
    
    def _on_display_loading_popup_event(self, event: Event):
        logger.debug(_("ViewEventController: получено событие display_loading_popup, транслируем в view", "ViewEventController: received event display_loading_popup, translating to view"))
        if self.view:
            self.view.display_loading_popup_signal.emit(event.data)
    
    def _on_hide_loading_popup_event(self, event: Event):
        logger.debug(_("ViewEventController: получено событие hide_loading_popup, транслируем в view", "ViewEventController: received event hide_loading_popup, translating to view"))
        if self.view:
