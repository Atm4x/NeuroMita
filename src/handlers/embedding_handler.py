...
def _ensure_torch_and_transformers() -> None:
    ...
    Установка выполняется заранее через UvSync active extras.
    """
    try:
        import torch
        from transformers import AutoModel, AutoTokenizer  # noqa: F401
        logger.info(
            f"torch loaded: version={torch.__version__}, "
            f"cuda_version={getattr(torch.version, 'cuda', None)}, "
            f"cuda_available={torch.cuda.is_available()}, path={getattr(torch, '__file__', '?')}"
        )
    except Exception as e:
        raise RuntimeError("torch/transformers не установлены. Запустите UvSync для core + backend extra.") from e
