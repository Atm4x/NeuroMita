...
class EditorMixin:
    ...
    def _apply_help_links(self, preset: dict) -> None:
        ...
        v.url_help_label.setText(f'<a href="{doc_url}" style="color: #ab5df5; text-decoration: underline;">{t("ui.settings.api.documentation")}</a>' if doc_url else "")

        v.model_help_label.setVisible(bool(models_url))
        v.model_help_label.setText(f'<a href="{models_url}" style="color: #ab5df5; text-decoration: underline;">{t("ui.settings.api.models_list")}</a>' if models_url else "")

        v.key_help_label.setVisible(bool(key_url))
        v.key_help_label.setText(f'<a href="{key_url}" style="color: #ab5df5; text-decoration: underline;">{t("ui.settings.api.get_api_key")}</a>' if key_url else "")
