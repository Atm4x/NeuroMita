...
def setup_model_interaction_controls(self, parent):
    ...
    data_management_config = [
        {'label': _('Запустить миграцию старых данных', 'Run old data migration'),
         'type': 'button',
         'command': lambda: get_event_bus().emit(Events.Migration.RUN_MIGRATION),
         'tooltip': _('Импортировать историю и память из старых JSON-файлов в базу данных.',
                       'Import history and memory from old JSON files into the database.')},
        {'label': _('Экспортировать данные персонажа', 'Export character data'),
         'type': 'button',
         'command': lambda: get_event_bus().emit(Events.Export.EXPORT_CHARACTER_DATA),
         'tooltip': _('Экспортировать историю и память текущего персонажа в текстовый файл.',
                       'Export current character\'s history and memory to a text file.')},
        {'label': _('Просмотр базы данных', 'View Database'),
         'type': 'button',
         'command': lambda: get_event_bus().emit(Events.GUI.SHOW_DB_VIEWER),
         'tooltip': _('Открыть окно для просмотра содержимого базы данных.',
                       'Open a window to view the contents of the database.')},
    ]

    create_settings_section(self, parent,
                           _("Управление данными", "Data Management"),
                           data_management_config)
